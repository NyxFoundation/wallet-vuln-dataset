...
|---|---:|
| 既定ブランチ上にある | 4,168 |
| 既定ブランチ外だがリリースタグに含まれる（出荷された） | 397 |
| **どのリリースにも届かなかった** | **892** |
| **squash マージで同じ修正が二重計上されている** | **475** |
| 図が読む母集団（出荷済みかつ重複でない） | **4,349** |

公開側 1,325 件のうち、コミット SHA を持ち判定できるのは 170 件で、**そのすべてが
到達済み**だった。残り 1,155 件は PR の URL で SHA を持たないため判定できず、`NA` の
まま残している。よって公開側は無補正で、比較の片側だけを削っていない。

比率は 107:1 ではなく **85:1**。結論の向きは変わらないが、件数は 2 割ほど過大だった。
3 つの図と README の数値はすべてこの補正後の 4,349 件で作り直してある。
フラグは [`scripts/mark_reachable.py`](../scripts/mark_reachable.py) が
`data/silent_mechanisms.csv` に列として書き込む（行は削除しない）。

`data/wallet_vulns.parquet` にはまだこの列がない。ゲートに反映するには
ローカルクローンへの依存を持たせる必要がある。
